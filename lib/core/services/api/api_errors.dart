...
abstract class Failure {
  ...
  Failure(this.msg, {this.statusCode});
}

class ServerFailure extends Failure {
  ServerFailure(super.msg, {super.statusCode});

  factory ServerFailure.generalException(exception) {
    try {
      if (exception is DioException) {
        return ServerFailure._fromDioException(exception);
      }
    } catch (e) {
  ...
  factory ServerFailure._fromDioException(DioException exception) {
    ...
    switch (exception.type) {
      ...
      case DioExceptionType.receiveTimeout:
        return ServerFailure(
          LocaleKeys.receiveTimeOutPleaseTryAgainLater.tr(),
          statusCode: exception.response?.statusCode,
        );
      case DioExceptionType.connectionError:
        return ServerFailure(
          LocaleKeys.connectionErrorPleaseTryAgainLater.tr(),
          statusCode: exception.response?.statusCode,
        );
      case DioExceptionType.cancel:
        return ServerFailure(
          LocaleKeys.requestHasBeenCancelledPleaseTryAgainLater.tr(),
          statusCode: exception.response?.statusCode,
        );
